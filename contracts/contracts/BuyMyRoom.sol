...
contract BuyMyRoom is ERC721Enumerable, Ownable {
    ...
    function buyHouse(uint256 tokenId) public payable {
        ...
        // require(msg.value == house.houseprice, "Incorrect value sent"); // 检查发送的以太币是否和房价一致

        // 打印当前房价和用户发送的金额，帮助调试
        // require(msg.value == house.houseprice, string(abi.encodePacked("Incorrect value sent, expected: ", house.houseprice)));
        ...
        // 将房款转给卖家，扣除手续费
        payable(house.owner).transfer(msg.value - platformFee);
        // 将手续费转给合约所有者
